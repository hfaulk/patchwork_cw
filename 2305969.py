...
from graphix import Window, Point, Rectangle, Line
...
def get_params() -> tuple[int, list]:
    ...
    while True:
        ...
        if (colours[0] not in valid_colours
            or colours[1] not in valid_colours
            or colours[2] not in valid_colours):
...
def pen_patch(window:Window,
    ...
    for y in range(top_left_y, top_left_y + patch_size, sub_patch_size):
        for x in range(top_left_x, top_left_x + patch_size, sub_patch_size):
            patch_components.append(draw_rect(window,
                                              Point(x, y),
                                              Point(x+sub_patch_size, 
                                                    y+sub_patch_size),
                                              "white",
                                              "black"))
            ...
            patch_components += sub_pen_patch(window,
                                              x,
                                              y,
                                              sub_patch_size,
                                              colour,
...
def sub_pen_patch(window:Window,
                  top_left_x:int,
                  top_left_y:int,
                  patch_size:int,
                  colour:str,
    ...
    if variant:
        ...
        for x in range(top_left_x, top_left_x + patch_size, rect_size):
            ...
            patch_components.append(draw_rect(window,
                                              Point(x, top_left_y),
                                              Point(x + rect_size,
                                                    top_left_y + patch_size),
                                              fill_colour,
                                              "black"))
    return patch_components

def fin_patch(window:Window,
              top_left_x:int,
              top_left_y:int,
              patch_size:int,
              colour:str) -> list:
    ...
    draw_rect(window,
              ...
              Point(top_left_x + patch_size, top_left_y + patch_size),
              "white",
              "black")
    ...
    for x in range(top_left_x, top_left_x + patch_size, line_sep):
        ...
        point_2:Point = Point(top_left_x + patch_size,
                              top_left_y + (count + line_sep))
    ...
    for y in range(top_left_y, top_left_y + patch_size, line_sep):
        ...
        point_2:Point = Point(top_left_x + (count + line_sep),
                              top_left_y + patch_size)
        patch_components.append(draw_line(window, point_1, point_2, colour))
        count += line_sep
    return patch_components
    
def pln_patch(window:Window,
              top_left_x:int,
              top_left_y:int,
              patch_size:int,
              colour:str) -> list:
...
def patchwork(window:Window,
    ...
    for x in range(0, patchwork_size, patch_size):
        ...
        for y in range(0, patchwork_size, patch_size):
            if (y == 0 and x != 0 
                or x == patchwork_size - patch_size 
                and y != x):
                patches[-1].append(fin_patch(window,
                                             x,
                                             y,
                                             patch_size,
                                             colours[1]))
            if ((x//patch_size) % 2 == 0):
                if (x <= y and x > patch_size
                    and y < patchwork_size - patch_size):
                    patches[-1].append(pen_patch(window,
                                                 x,
                                                 y,
                                                 patch_size,
                                                 colours[0]))
                elif (y >= x):
                    patches[-1].append(pln_patch(window,
                                                 x,
                                                 y,
                                                 patch_size,
                                                 colours[0]))
                elif (y >= patch_size
                      and x < patchwork_size - patch_size):
                    patches[-1].append(pln_patch(window,
                                                 x,
                                                 y,
                                                 patch_size,
...
def outline_patch(window:Window,
    ...
    for i in range(len(corners)):
        ...
        outline.append(draw_line(window,
                                 Point(corners[i][0],
                                       corners[i][1]),
                                 Point(corners[next_i][0],
                                       corners[next_i][1]),
                                 "black",
...
def find_adjacent_empty_patch(all_patches:list,
                              selected_patch_index:list[int, int],
                              direction:str) -> list[int, int]:
    directions:dict = {"Left":  [i for i in range(selected_patch_index[0]-1,
                                                  -1,
                                                  -1)],
                       "Right": [i for i in range(selected_patch_index[0]+1,
                                                  len(all_patches),
                                                  1)],
                       "Up":    [i for i in range(selected_patch_index[1]-1,
                                                  -1,
                                                  -1)],
                       "Down":  [i for i in range(selected_patch_index[1]+1,
                                                  len(
                                                      all_patches[
...
def move_patch(window:Window,
               patches:list,
               selected_index:list[int ,int],
               new_index:list[int, int]) -> list:
    ANIM_STEP = 10
    ...
    # \/ x100 to both coords to scale up to size of tiles
    delta = [i*100 for i in delta]
    ...
    for i in range(ANIM_STEP):
        for component in selected_patch_components:
            #Have to redraw with each movement to remain on top
            component.undraw()
            component.move(*increment)
            component.draw(window)
        sleep(0.01)
        
    patches[new_index[0]][new_index[1]] = selected_patch_components
...
def handle_digit(window:Window,
                 selected_patch:list,
                 key:str,
                 patches:list,
                 rounded_x:int,
    ...
    if selected_patch == []:
        ...
        colour = colours[digit_keys[key][1]]
        function = digit_keys[key][0]
        patches[rounded_x//100][rounded_y//100] = function(window,
                                                           rounded_x,
                                                           rounded_y,
                                                           patch_size,
                                                           colour)
...
def handle_action(window:Window,
                  selected_patch:list,
                  key:str,
                  patches:list,
                  rounded_x:int,
                  rounded_y:int,
                  outline:list) -> list:
    action_keys:dict = {"x": undraw_patch,
                        "Escape": undraw_patch,
                        "Left": move_patch,
                        "Right": move_patch,
                        "Up": move_patch,
    ...
    if key in ["Left", "Right", "Up", "Down"]:
        if selected_patch != []:
            adjacent_coords = find_adjacent_empty_patch(patches,
                                                    [rounded_x, rounded_y],
                                                    key)
            if adjacent_coords != None: 
                patches = action_keys[key](window,
                                           patches,
                                           [rounded_x, rounded_y],
                                           adjacent_coords)
    elif key == "x":
...
def main() -> None:
    ...
    while True:
        ...
        patches, click_data["outline"] = handle_key(win,
                                                    patches,
                                                    key,
                                                    click_data["x"],
                                                    click_data["y"],
